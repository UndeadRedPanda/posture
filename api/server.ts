...
import { cors, logger, timer } from "./middlewares.ts";
import { getValue } from "../utils/mod.ts";
import { Status } from "https://deno.land/std@0.88.0/http/http_status.ts";
  ...
  private _setupRoutes() {
    this.router
      .get("/api/v1/messages", async (context) => {
        ...
        const data = await this._database.getMessages(
